...
namespace Jarvis.COM
{
    ...
    public class SocketCom : IConnectable, ICom
    {
        #region Events
        ...
        protected void InvokeConnectionChanged(ConnectionStatusType status)
        {
            ...
            ConnectionChanged?.Invoke(this, new ConnectionChangedEventArgs(status, _hostOrIp));
        }
        ...
        private readonly SocketComType _type;
        ...
        private void IniCom()
        {
            switch (_type)
            {
                case SocketComType.Server:
                    IPAddress ipAddress = System.Net.IPAddress.Parse(_hostOrIp);
        ...
        private async Task<bool> StartClient(KeyValuePair<TcpClient, byte[]> client)
        {
            ...
            try
            {
                if (_type == SocketComType.Client)
                {
                    await client.Key.ConnectAsync(_hostOrIp, _port);
                }
                client.Key.Client.BeginReceive(client.Value, 0, client.Value.Length, SocketFlags.None, OnDataReceived, client);
