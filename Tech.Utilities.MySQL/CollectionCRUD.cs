...
namespace Jarvis.Utilities.MySQL
{
    internal partial class CollectionCRUD<T> : ICollectionCRUD<T> where T : IId
    {
        ...
        protected void InvokeErrorOccurred(string value) => ErrorOccurred?.Invoke(this, new EventArgs<string>(value));
        ...
        public async Task<long> Delete(IEnumerable<IQueryFilter>? filters = null)
        {
            try
            {
                string deleteQuery = string.Format(CultureInfo.InvariantCulture, "DELETE FROM {0} ", TableName);
                MySqlCommand deleteCmd = new(deleteQuery);
                if (filters != null && AppendFilters(filters, deleteCmd))
                {
                    deleteCmd.CommandText += "\nSelect @@ROWCOUNT";
                    int res = await Utilities.ExecuteScalar(_connection, deleteCmd);
                    return res;
                }
                return 0;
            }
