...
namespace Jarvis.Utilities.MongoDB
{
    public class DbCollection<T> : DbCollectionBase<T> where T : IId
    {
        ...
        /// </summary>
        public string? CollectionName => Settings.Name;
        ...
        /// </summary>
        /// <param name="settings">The <see cref="DatabaseSettings"/> used to initializes the object.</param>
        public DbCollection(IDbCollectionSettings settings) : base(settings)
        {
            if (settings is not DbCollectionSettings)
            {
                throw new ArgumentException($"{nameof(settings)} should be of type {nameof(DbCollectionSettings)}.", nameof(settings));
            }
            if (settings.DatabaseSettings is not DatabaseSettings)
            {
                throw new ArgumentException($"{nameof(settings.DatabaseSettings)} should be of type {nameof(DatabaseSettings)}.");
            }
        }
        ...
        {
            if (_client != null && Settings != null && Settings.DatabaseSettings != null && Settings.DatabaseSettings.DatabaseName != null && Settings.Name != null)
            {
                CollectionCRUD = new CollectionCRUD<T>(_client, Settings.DatabaseSettings.DatabaseName, Settings.Name);
                CollectionCRUD.ErrorOccurred += (s, e) =>
                {
                    InvokeErrorOccurred(e.Value);
                };
            }
        }
        ...
        protected override void Init()
        {
            ...
            if (_client == null && Settings != null && Settings.DatabaseSettings is DatabaseSettings settings && Settings.DatabaseSettings.ConnectionString != null)
            {
#pragma warning disable IDE0079 // Remove unnecessary suppression
#pragma warning disable CS0612 // Type or member is obsolete
                MongoClientSettings mongoSettings = GetSettings(Settings.DatabaseSettings.ConnectionString, settings.NetworkAddresses);
...
#pragma warning restore IDE0079 // Remove unnecessary suppression
        ...
        protected override async void Listen()
        {
            ...
            if (_client != null && Settings != null && Settings.DatabaseSettings != null)
            {
                IMongoCollection<T> collection = _client.GetDatabase(Settings.DatabaseSettings.DatabaseName).GetCollection<T>(Settings.Name);
                try
                {
                    if (collection != null)
                    {
                        ...
                        await cursor.ForEachAsync(change =>
                        {
                            switch (change.OperationType)
                            {
                                case ChangeStreamOperationType.Insert:
                                    InvokeCollectionChanged(new CollectionChangedEventArgs(CollectionChangedEventArgs.CollectionChange.ElementAdded, change.FullDocument));
                                    break;

                                case ChangeStreamOperationType.Update:
                                    InvokeCollectionChanged(new CollectionChangedEventArgs(CollectionChangedEventArgs.CollectionChange.ElementUpdated, change.FullDocument));
                                    break;
                                ...
                                case ChangeStreamOperationType.Delete:
                                    ...
                                    InvokeCollectionChanged(new CollectionChangedEventArgs(CollectionChangedEventArgs.CollectionChange.ElementRemoved, id));
                                    break;
